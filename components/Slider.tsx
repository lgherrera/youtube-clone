...
export default function Slider({ videos }: SliderProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const cardWidth = container.offsetWidth * 0.9; // Matches the 90% width below
      const currentScroll = container.scrollLeft;
      const maxScroll = container.scrollWidth - container.clientWidth;

      // Reset to beginning if at the end, otherwise scroll to next
        ...
        {videos.map((video) => (
          <div key={video.id} className="min-w-[90%] snap-center">
            <Link href={`/video/${video.id}`} className="block relative group">
              {/* Using 16:9 ratio for consistency */}
